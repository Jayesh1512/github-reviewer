...
| ------------------- | ------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------- |
| Framework           | **Next.js 16 (App Router)**           | Server components for initial load performance. API routes for backend. Mature ecosystem.                                              |
...
1. Implement GitHub OAuth via NextAuth.js v5
2. Create user table and sync GitHub profile on login
3. Add auth middleware to protect API routes
4. Build login/logout UI

### Epic 3: PR Data Layer

1. Build GitHub API client (typed, with caching)
2. Implement PR list endpoint (user's review-requested + authored PRs)
3. Implement PR metadata endpoint (title, description, reviewers, status)
4. Implement file list endpoint with categorization logic
5. Implement diff/patch endpoint with hunk parsing

### Epic 4: File Sidebar

1. Build collapsible file tree component
2. Implement directory/type/impact grouping logic
3. Add file review status indicators
4. Add search/filter for file tree

### Epic 5: Diff Viewer

1. Build base diff renderer (unified + split view)
2. Integrate Shiki for syntax highlighting
3. Implement virtual scrolling with TanStack Virtual
4. Build collapsible hunks
5. Add minimap component
6. Implement keyboard navigation (j/k/n/p)

### Epic 6: Comments & Review

1. Build inline comment UI (add, view, reply)
2. Implement comment sync to GitHub API
3. Build review submission flow (approve/request changes/comment)
4. Add local notes feature (not synced to GitHub)

### Epic 7: AI Assistance Bar

1. Build floating AI bar component (collapsed/expanded states)
2. Implement code selection context detection
3. Build AI prompt construction with PR context
4. Implement streaming response display
5. Add quick-action buttons
6. Add "post as comment" action for AI responses

### Epic 8: Review Session Management

1. Create review_sessions and file_review_statuses tables
2. Implement session creation/resume logic
3. Build review progress UI (progress bar, file counts)
4. Add session recovery on page reload
